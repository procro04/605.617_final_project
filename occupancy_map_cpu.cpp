 ...
 *
 * Design decisions for CUDA-readiness:
 *   - Flat 1D array backing the 2D grid (row-major). No pointers-of-pointers,
 *     no tree nodes — just contiguous memory that can be copied to device.
 *   - Bresenham ray marching operates on integer grid indices, avoiding
 *     floating-point step accumulation. This maps cleanly to GPU threads.
 *   - All core update functions take raw pointers + dimensions so they can
 *     later become __device__ / __global__ functions with minimal changes.
 *   - Timing hooks already in place for the performance analysis.
 *
 * Build:
 *   g++ -O2 -std=c++17 -o occupancy_map_cpu occupancy_map_cpu.cpp
 *
 * Usage:
 ...
 *
 * Author: Zachary Crowe — EN 605.617 Project
 */
...
std::vector<LidarScan> load_lidar_dataset(const std::string& filepath) {
    ...
    while (std::getline(file, line)) {
        ...
        if (!(iss >> scan.pose.x >> scan.pose.y >> scan.pose.theta)) continue;
...
class OccupancyGrid {
public:
    /// @param grid_size_meters  Physical side length of the square grid.
    /// @param resolution        Meters per grid cell.
    /// @param origin_x/y        World coordinate of the grid center.
    ...
    void ray_update(float ox, float oy, float hx, float hy) {
        int c0, r0, c1, r1;
        if (!world_to_grid(ox, oy, c0, r0)) return;
        if (!world_to_grid(hx, hy, c1, r1)) return;
    ...
    bool write_pgm(const std::string& filepath) const {
        ...
        for (int r = num_cells_ - 1; r >= 0; --r) {   // flip Y so up=north
            for (int c = 0; c < num_cells_; ++c) {
                int8_t v = cells_[idx(c, r)];
                uint8_t pixel;
                switch (v) {
                    case OCCUPIED:   pixel = 0;   break;
                    case UNOCCUPIED: pixel = 255; break;
                    default:         pixel = 128; break;  // UNKNOWN
                }
...
private:
    ...
    ///
    /// @param grid       Pointer to the flat grid array.
    /// @param width      Number of cells per row (grid is width x width).
    /// @param c0,r0      Start cell (robot position).
    /// @param c1,r1      End cell (LIDAR hit).
...
int main(int argc, char* argv[]) {
    ...
    for (const auto& s : scans) {
        ...
        for (const auto& [hx, hy] : s.hits) {
            min_x = std::min(min_x, hx); max_x = std::max(max_x, hx);
            min_y = std::min(min_y, hy); max_y = std::max(max_y, hy);
        }
    }
